...
const labels = `PELVISO			PELVISA			PELVISL			PELVISP			LFEMURO			LFEMURA			LFEMURL			LFEMURP			LTIBIAO			LTIBIAA			LTIBIAL			LTIBIAP			LFOOTO			LFOOTA			LFOOTL			LFOOTP			LTOESO			LTOESA			LTOESL			LTOESP			RFEMURO			RFEMURA			RFEMURL			RFEMURP			RTIBIAO			RTIBIAA			RTIBIAL			RTIBIAP			RFOOTO			RFOOTA			RFOOTL			RFOOTP			RTOESO			RTOESA			RTOESL			RTOESP			THORAXO			THORAXA			THORAXL			THORAXP			LUPPERARMO			LUPPERARMA			LUPPERARML			LUPPERARMP			LFOREARMO			LFOREARMA			LFOREARML			LFOREARMP			LHANDO			LHANDA			LHANDL			LHANDP			RUPPERARMO			RUPPERARMA			RUPPERARML			RUPPERARMP			RFOREARMO			RFOREARMA			RFOREARML			RFOREARMP			RHANDO			RHANDA			RHANDL			RHANDP			HEADO			HEADA			HEADL			HEADP			LFHD			RFHD			LBHD			RBHD			LMAS			RMAS			GLAB			C7			T2			T10			CLAV			STRN			RBAK			LSHO			LUPA			LELB			LFRM			LWRA			LWRB			LFIN			RSHO			RUPA			RELB			RFRM			RWRA			RWRB			RFIN			LASI			RASI			LPSI			RPSI			LTHI			LTHAP			LTHAD			LKNE			LTIB			LTIAP			LTIAD			LANK			LHEE			LTOE			LFMH			LVMH			LFootOff			RTHI			RTHAP			RTHAD			RKNE			RTIB			RTIAP			RTIAD			RANK			RHEE			RTOE			RFMH			RVMH			RFootOff`;
const client = new XMLHttpRequest();
client.open('GET', '/data.csv')

let points_arr = [] ;
client.onreadystatechange = () => {
  if(client.responseText) points_arr = client.responseText.split('\n').map(t => t.split(/\s/));
  drawFrame()
}
client.send();

const labels_arr = labels.split(/\s/);
...
var debug = true;
  ...
  function addPoint(x,y,z, name, main) {
    const p_geo = new THREE.CircleGeometry(0.1, 64);
    const p_mat = new THREE.MeshBasicMaterial({color: new THREE.Color(1, !!main, 0)});
   
    const point = new THREE.Mesh(p_geo, p_mat);
    point.position.set(x, y, z);
    points[name] = point.id;
    lookaters.push(point.id)
    return point;
  }
  ...
  function init( width, height) {
    ...
    labels_arr.forEach((label, id) => scene.add(addPoint(0,0,0, label)));
    scene.add(addPoint(0,0,0, 'A', true));
  ...
  let animationStep = 0
  function drawFrame() {
    const step = animationStep % points_arr.length
    if(!points_arr.length) return;
    const scale = 0.01
    
    labels_arr.forEach((label, id) => scene.getObjectById(points[label]).position.set(scale*points_arr[step][id*3], scale*points_arr[step][id*3+2], scale*points_arr[step][id*3+1]))
  }
  ...
  function stare(obj){
    obj.lookAt(new THREE.Vector3(camera.position.x, camera.position.y, camera.position.z));
  }
